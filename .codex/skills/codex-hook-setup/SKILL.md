---
name: codex-hook-setup
description: Use when setting up, repairing, verifying, or troubleshooting Codex hooks in this project, especially UserPromptSubmit logging via the local .codex directory, trusted mode checks, hooks.json command wiring, and git user name prompt-prefix logging.
---

# Codex Hook Setup

Use this skill when the user wants to install Codex hooks from scratch, repair a broken setup, check why a hook is not firing, or verify that prompt logs are being written correctly for this repository.

## Workflow

1. Inspect the local hook files before proposing changes:
   - `.codex/config.toml`
   - `.codex/hooks.json`
   - the referenced script under `.codex/hooks/`
2. Verify whether the project must be opened in trusted mode for `.codex` to load.
...
## What Good Hook Setups Usually Include
...
- a project-local log path such as `logs/user-prompts.jsonl`
...
- If the structure is correct but runtime behavior still fails, explicitly tell the user to reopen the project in trusted mode because that cannot be repaired by editing repository files alone.
...
- Use `scripts/install_or_repair_hook.py` to write the standard project-local prompt hook files.
