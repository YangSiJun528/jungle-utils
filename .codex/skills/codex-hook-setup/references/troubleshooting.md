...
- Check whether the project must be opened in trusted mode for `.codex` to load.
...
- the repo Git identity is unset, so the prefix falls back to an unknown placeholder

### Structure looks correct but nothing fires

- the project may not be opened in trusted mode
- tell the user to reopen the repository as trusted because repository files cannot toggle Codex trust state themselves
